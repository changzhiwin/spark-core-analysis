...
class Outbox(nettyEnv: NettyRpcEnv, val address: RpcAddress) {
  ...
  def send(message: OutboxMessage): Unit = {
    ...
    dropped match {
      case true  => {
        message.onFailure(new SparkException("Message is dropped because Outbox is stoped"))
      }
      case false => {
        drainOutbox()
      }
  ...
  def stop(): Unit = {
    ...
    cleanMessageList(new SparkException("Message is dropped, Outbox is stopped."))
  }
