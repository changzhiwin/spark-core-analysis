...
private[spark] object ClosureCleaner extends Logging {
  ...
  private def clean(
    ...
    if (!isClosure(func.getClass) && maybeIndylambdaProxy.isEmpty) {
      logger.debug(s"Expected a closure; got ${func.getClass.getName}")
      return
    }
    ...
    if (maybeIndylambdaProxy.isEmpty) {
      ...
      // log.isDebugEnabled
      if (false) {
        logger.debug(s" + declared fields: ${declaredFields.size}")
        declaredFields.foreach { f => logger.debug(s"     $f") }
        logger.debug(s" + declared methods: ${declaredMethods.size}")
        declaredMethods.foreach { m => logger.debug(s"     $m") }
        logger.debug(s" + inner classes: ${innerClasses.size}")
        innerClasses.foreach { c => logger.debug(s"     ${c.getName}") }
        logger.debug(s" + outer classes: ${outerClasses.size}" )
        outerClasses.foreach { c => logger.debug(s"     ${c.getName}") }
      }
      ...
      if (accessedFields.isEmpty) {
        logger.debug(" + populating accessed fields because this is the starting closure")
      ...
      logger.debug(s" + fields accessed by starting closure: ${accessedFields.size} classes")
      ...
      if (outerPairs.nonEmpty) {
        ...
        if (isClosure(outermostClass)) {
          logger.debug(s" + outermost object is a closure, so we clone it: ${outermostClass}")
          ...
          logger.debug(s" + outermost object is a REPL line object, so we clone it:" +
            s" ${outermostClass}")
        } else {
          ...
          logger.debug(s" + outermost object is not a closure or REPL line object," +
      ...
      for ((cls, obj) <- outerPairs) {
        logger.debug(s" + cloning instance of class ${cls.getName}")
        ...
        if (cleanTransitively && isClosure(clone.getClass)) {
          logger.debug(s" + cleaning cloned closure recursively (${cls.getName})")
      ...
      logger.debug(s"Cleaning indylambda closure: $implMethodName")
      ...
      if (needsCleaning) {
        ...
        if (accessedFields(capturingClass).size < capturingClass.getDeclaredFields.length) {
          ...
          logger.debug(s" + cloning instance of REPL class $capturingClassName")
      ...
      logger.debug(s" +++ indylambda closure ($implMethodName) is now cleaned +++")
    }
...
private[spark] object IndylambdaScalaClosures extends Logging {
    ...
    maybeClosure.getClass match {
      ...
      case c if isClosureCandidate(c) =>
        try {
          ...
          case e: Exception =>
            logger.debug("The given reference is not an indylambda Scala closure.", e)
            None
        }
  ...
  def findAccessedFields(
    ...
    while (!stack.isEmpty) {
      ...
      logger.trace(s"  scanning ${currentId.cls.getName}.${currentId.name}${currentId.desc}")
      currentMethodNode.accept(new MethodVisitor(ASM9) {
        ...
        override def visitFieldInsn(op: Int, owner: String, name: String, desc: String): Unit = {
          if (op == GETFIELD || op == PUTFIELD) {
            ...
            for (cl <- accessedFields.keys if cl.getName == ownerExternalName) {
              logger.trace(s"    found field access $name on $ownerExternalName")
        ...
        override def visitMethodInsn(
          ...
          if (owner == currentClassInternalName) {
            logger.trace(s"    found intra class call to $ownerExternalName.$name$desc")
            ...
            logger.debug(s"    found inner class $ownerExternalName")
            ...
            for (m <- innerClassNode.methods.asScala) {
              logger.trace(s"      found method ${m.name}${m.desc}")
            ...
            logger.trace(s"    found call to outer $ownerExternalName.$name$desc")
            ...
            logger.trace(s"    ignoring call to $ownerExternalName.$name$desc")
          }
        }
        ...
        override def visitInvokeDynamicInsn(
          ...
          logger.trace(s"    invokedynamic: $name$desc, bsmHandle=$bsmHandle, bsmArgs=$bsmArgs")
          ...
          if (isLambdaMetafactory(bsmHandle)) {
            ...
            if (isLambdaBodyCapturingOuter(targetHandle, currentClassInternalName)) {
              ...
              logger.debug(s"    found inner closure $targetHandle")
              val calleeMethodId =
